...
### Fixed
- **`PEEK_FILES_ROOT` bypass via symlinked parent (security).** When a target file didn't yet exist (typical for `download_to` / `save_to`), `canonicalize` fell back to lexical `path.resolve` — meaning `<root>/symlinked-dir/new.txt` could pass `isUnderRoot` while the actual write landed outside `PEEK_FILES_ROOT`. `canonicalize` now walks up to the nearest existing ancestor, `realpathSync`s that, and re-attaches the non-existent suffix. Implementation uses `path.dirname` + `path.parse(abs).root` so it works on POSIX, Windows drive roots (`C:\\`), and UNC shares.
...
### Changed
- **`meta.body_inclusion.inline_cap_bytes` → `inline_body_cap_bytes`** (and same key in `error.detail` on `body_too_large_for_inline`). Now matches the env var (`PEEK_INLINE_BODY_CAP`) and the `server_info.effective_limits.inline_body_cap_bytes` field. Wire-level rename — safe right now (0 installs).
...
### Spec
- `MAX_RESPONSE_BYTES` → `PEEK_MAX_RESPONSE_BYTES` in §6 and §9.
...
### Added
- **`body_mode: "head"`** — schema + `body_preview` with arrays/strings truncated and sibling `_truncated` markers. `auto` upgrades through head between `PEEK_INLINE_THRESHOLD_BYTES` and `PEEK_HEAD_PREVIEW_THRESHOLD`.
...
- **`multipart.files[].content_base64`** — alternative file payload mode. The bytes travel inline in the JSON-RPC frame, capped by `PEEK_MAX_INLINE_FILE_BYTES` (default 10 MB). Recommended for remote-MCP scenarios where path semantics differ between agent and server. `PEEK_FILES_ROOT` does not apply (no path).
- **New error kinds** — `body_too_large_for_inline` (with `cache_id` in `detail` so agent can pivot to `http_read`), `unsupported_field`.
- **New env vars** — `PEEK_HEAD_PREVIEW_THRESHOLD` (64 KB), `PEEK_HEAD_PREVIEW_ITEMS` (5), `PEEK_HEAD_PREVIEW_STRING` (200), `PEEK_INLINE_BODY_CAP` (256 KB), `PEEK_MAX_INLINE_FILE_BYTES` (10 MB). `PEEK_AUTO_INCLUDE_BODY_BYTES` is **renamed** to `PEEK_INLINE_THRESHOLD_BYTES` (same default 8192).
...
- **Symlink escape** — `ensureUnderRoot` resolves through `realpathSync` first (lexical fallback for non-existent paths) so symlinks can't escape `PEEK_FILES_ROOT`.
...
- jq filtering via `jq-wasm` (with `PEEK_USE_NATIVE_JQ` env-toggle reserved for future native fallback).
- In-memory cache with 10-minute TTL.
- `download_to` for streaming binary responses straight to disk (sha256 computed inline).
- Configurable via `PEEK_*` env vars (timeouts, size caps, schema depth/key limits, TLS).
- `PEEK_FILES_ROOT` env: when set, restricts `multipart.files[].path`, `download_to`, and `save_to` to canonical paths under this prefix; surfaces the constraint in tool descriptions for the agent. Unset = no constraint (npm-mode default).
- Unified error envelope with stable `kind` codes.
- npm + Docker (alpine multistage) distribution. Recommended Docker pattern is a same-path bind mount paired with `PEEK_FILES_ROOT` — agent paths "just work" without host↔container translation.
